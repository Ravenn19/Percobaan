...
<head>
    ...
    <style>
        .proposal-action-buttons a {
            margin-right: 10px;
        }

        .proposal-action-buttons a:last-child {
            margin-right: 0;
        }
            ...
            <li class="nav-item">
                ...
                <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item active" href="{{ route('admin.proposals') }}">Proporsal</a>
            ...
            <div id="content">
                ...
                <!-- Begin Page Content -->
                    <div class="container mt-4">
                        ...
                        <div class="row">
                            @forelse($usersWithProposals as $user)
                            <div class="col-md-6 mb-4">
                                <div class="card p-3 shadow-sm">
                                    <h5>{{ $user->nama_tim }}</h5>
                                    <p>Ketua: {{ $user->ketua }}</p>

                                    @if($user->proposal_path)
                                        <div class="border p-2 rounded mb-2" style="background: #f8f9fa;">
                                            <strong>Preview Proposal:</strong>
                                        </div>

                                        <div class="mb-2">
                                            <a href="{{ route('admin.download', ['id' => $user->id, 'type' => 'proposal']) }}" class="btn btn-primary btn-sm" target="_blank">
                                                Download Proposal
                                            </a>
                                        </div>

                                        {{-- Approve/Unapprove Button --}}
                                        <div class="proposal-action-buttons d-flex">
                                            <a href="{{ route('admin.toggleProposal', ['id' => $user->id, 'action' => 'approve']) }}" class="btn btn-success btn-sm me-3">✔ Approve</a>
                                            <a href="{{ route('admin.toggleProposal', ['id' => $user->id, 'action' => 'unapprove']) }}" class="btn btn-danger btn-sm">× Unapprove</a>
                                        </div>

                                        @if($user->proposal_approved === 1)
                                            <p class="text-success mt-2">✅ Telah Disetujui</p>
                                        @elseif($user->proposal_approved === 0)
                                            <p class="text-danger mt-2">❌ Tidak Disetujui</p>
                                        @else
                                            <p class="text-muted mt-2">⏳ Belum Ditinjau</p>
                                        @endif
                                    @else
                                        <p class="text-muted">Proposal belum diupload.</p>
                                    @endif
                                </div>
                            </div>
                            @empty
